...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [client, invoices, todos, portfolio, socialPosts, assets] = await Promise.all([
    ...
    prisma.asset.findMany({ where: { clientId: id, uploadedBy: user.id }, orderBy: { createdAt: "desc" } }),
  ]);

  if (!client) return NextResponse.json({ error: "Introuvable" }, { status: 404 });

  return NextResponse.json({ client, invoices, todos, portfolio, socialPosts, assets });
}
